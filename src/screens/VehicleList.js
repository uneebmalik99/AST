            ...
            <Appbar
                    ...
                    <TouchableOpacity 
                                style={{alignContent:"center", alignItems:"center", justifyContent:'center'}}
                                            //   onPress={() => this.props.navigation.navigate('LoginScreen')}
                    >
                  <Text style={{fontSize:16, fontWeight:'600'}}>{this.state.statusId == '1'? 'ON HAND' : this.state.statusId == '2'? 'MANIFEST': this.state.statusId == '3'? 'ON THE WAY' :this.state.statusId == '4'? 'SHIPPED' : this.state.statusId == '5'? 'PICKED UP'  :this.state.statusId == '6'? 'ARRIVED':this.state.statusId == '7'? 'HANDED OVER': 'All VEHICLES'  }</Text>
